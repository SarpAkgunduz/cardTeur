...
- `MatchPage.tsx` currently bypasses `playerApi` for the match announce call and uses `fetch('http://localhost:5002/api/match/announce')` directly. Default backend port is `5002` (changed from 5001 due to macOS AirPlay Receiver conflict). If you change API URLs or error handling, update both `openteur/src/services/api/apiClient.ts` and `openteur/src/pages/MatchPage.tsx`.
